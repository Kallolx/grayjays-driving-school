  ...
  return (
    <div className="fixed bottom-24 right-6 w-96 z-50">
      <div className="bg-white rounded-2xl shadow-2xl transform transition-all duration-300 animate-slideUp">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#2c3149] to-[#1a1f33] p-4 rounded-t-2xl relative">
          <button 
            onClick={onClose}
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-yellow-500/20 rounded-xl flex items-center justify-center">
              <img src="/icons/svg-image-1.svg" alt="Grayjays" className="w-6 h-6" />
            </div>
            <div>
              <h3 className="font-bold text-white">Grayjays Assistant</h3>
        ...
        <div className="h-96 overflow-y-auto p-4 space-y-4">
          {messages.map((message, index) => (
            ...
            >
              {message.type === "bot" && (
                <div className="w-8 h-8 rounded-xl bg-yellow-500/10 flex items-center justify-center mr-2">
                  <img src="/icons/svg-image-1.svg" alt="Grayjays" className="w-4 h-4" />
                </div>
              )}
              <div
              ...
              <div className="grid grid-cols-2 gap-2">
                {quickActions.map((action, index) => (
                  <button
                    ...
                    className="flex items-center gap-2 p-2 text-sm text-[#2c3149] bg-gray-100 rounded-xl hover:bg-yellow-500 transition-colors group"
                  >
                    <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center group-hover:bg-yellow-400 transition-colors">
                      {action.icon}
                    </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 rounded-xl border-2 border-gray-100 focus:border-yellow-500 focus:outline-none text-sm transition-colors"
            />
            <button
              onClick={() => handleSend()}
